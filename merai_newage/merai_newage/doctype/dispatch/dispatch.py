...
class Dispatch(Document):
    def on_submit(self):
        ...
        if not robot_tracker_name:
            frappe.throw("Robot Tracker not found for this Work Order & Batch No.")
...
def update_dispatch_details(doc):
    ...
    # 1️⃣ Resolve Work Order from Batch
    wo = frappe.db.get_value("Batch", {"name": batch_no}, "reference_name")
    if not wo:
    ...
    # 2️⃣ Fetch job card items for that Work Order
...
def get_available_batches(doctype, txt, searchfield, start, page_len, filters):
    ...
    if not exclude:
        exclude = [""]  # avoid empty tuple error
