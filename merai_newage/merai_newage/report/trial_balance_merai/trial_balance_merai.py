...
def get_data(filters):
	accounts = frappe.db.sql(
		"""select name, account_number, parent_account, account_name, root_type, is_group, report_type, lft, rgt
	...
	# Accumulate values BEFORE filtering
...
def calculate_total_row(all_accounts, company_currency):
	"""Calculate totals from all root-level accounts (before any filtering)"""
	total_row = {
		"account": "'" + _("Total") + "'",
		...
		"accumulated_opening_balance": 0.0,
		"accumulated_closing_balance": 0.0,
	}

	# Sum only root-level accounts (accounts with no parent)
	for d in all_accounts:
		if not d.get("parent_account"):
			for field in value_fields:
				total_row[field] += flt(d.get(field, 0))
			
			# Calculate accumulated balances
			opening_balance = flt(d.get("opening_debit", 0)) - flt(d.get("opening_credit", 0))
			closing_balance = flt(d.get("closing_debit", 0)) - flt(d.get("closing_credit", 0))
			
			total_row["accumulated_opening_balance"] += opening_balance
			total_row["accumulated_closing_balance"] += closing_balance
...
def prepare_data(accounts, filters, parent_children_map, company_currency, all_accounts_before_filter):
	data = []
	...
	for d in accounts:
		...
		# ---- Accumulated Opening Balance ----
		row["accumulated_opening_balance"] = (
			flt(row.get("opening_debit", 0)) - flt(row.get("opening_credit", 0))
		)

		# ---- Accumulated Closing Balance ----
		row["accumulated_closing_balance"] = (
			flt(row.get("closing_debit", 0)) - flt(row.get("closing_credit", 0))
		)

		row["has_value"] = has_value
		data.append(row)

	# Calculate total using ALL accounts before filtering
	total_row = calculate_total_row(all_accounts_before_filter, company_currency)
...
def get_columns():
	return [
		...
		{
			"fieldname": "accumulated_opening_balance",
			"label": _("Accumulated Opening Balance"),
			"fieldtype": "Currency",
			"options": "currency",
			"width": 160,
		},
		...
		{
			"fieldname": "accumulated_closing_balance",
			"label": _("Accumulated Closing Balance"),
			"fieldtype": "Currency",
			"options": "currency",
			"width": 160,
		},
