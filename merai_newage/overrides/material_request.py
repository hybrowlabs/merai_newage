...
#     data = frappe.db.sql(""" select wo.wip_warehouse, woi.source_warehouse from `tabWork Order` as wo
#                             Inner Join `tabWork Order Item` as woi on woi.parent = wo.name
#                             where wo.name = %s
#                         """,(work_order_name), as_dict=True)
    
#     if data[0]['wip_warehouse'] == None or data[0]['source_warehouse'] == None:
#         return "Failure"
    
#     frappe.db.set_value("Material Request", name, "set_from_warehouse", data[0]['source_warehouse'])
#     frappe.db.set_value("Material Request", name, "set_warehouse", data[0]['wip_warehouse'])
