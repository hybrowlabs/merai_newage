...
def before_validate_purchase_receipt(doc, method):
    validate_qi(doc)
def validate_purchase_receipt(doc, method):
    print("Validating Purchase Receipt for Asset Creation Request...76")
...
def before_submit(doc, method=None):
    print("Before submit called for Purchase Receipt:========================", doc.name)
    link_qi_and_ar_no(doc)
...
def create_new_cwip_asset(pr_doc, acr, asset_category, pr_amount, total_cwip_amount):
    ...
    asset_doc = frappe.get_doc({
        ...
        "gross_purchase_amount": 0 if is_composite else total_cwip_amount,
        "supplier": pr_doc.supplier,
        "purchase_receipt": pr_doc.name,
        "purchase_receipt_item": main_item.name,  # ✅ ADDED: exact PR item row
        
        # Reference fields
        "custom_asset_creation_request": acr.name,
        "is_composite_asset": 1 if is_composite else 0,
...
def create_assets_from_asset_masters(pr_doc):
    ...
    ✅ FIXED: purchase_receipt_item correctly mapped per PR item row using zip
    ...
    # Group PR items by item_code — preserving insertion order
    pr_items_by_code = {}
    for item in pr_doc.items:
    ...
    # Process each item_code group separately
    for item_code, pr_items_list in pr_items_by_code.items():
        ...
        if len(asset_masters) < assets_needed and unassigned_masters:
            ...
            for am in masters_to_assign:
                frappe.db.set_value("Asset Master", am["name"], {
                    "item": item_code,
                    "item_name": pr_items_list[0].item_name
                }, update_modified=False)
                am["item"] = item_code
                am["item_name"] = pr_items_list[0].item_name
                asset_masters.append(am)
        ...
        # ✅ FIX: zip asset_masters with pr_items_list so each asset gets its own PR item row
        # If fewer PR rows than assets (bulk qty case), pad with last PR row
        padded_pr_items = pr_items_list + [pr_items_list[-1]] * max(0, len(asset_masters) - len(pr_items_list))
        paired = list(zip(asset_masters, padded_pr_items))

        for idx, (am, pr_item_row) in enumerate(paired, 1):
            try:
                item_code_to_use = am.get("item") or item_code
                item_name_to_use = am.get("item_name") or pr_items_list[0].item_name
                ...
                cost_per_asset = get_asset_cost(am, pr_item_row, asset_qty)
                ...
                print(f"PR item row: {pr_item_row.name}")  # ✅ debug log
                ...
                asset_doc = frappe.get_doc({
                    ...
                    # "purchase_receipt_item": pr_item_row.name,  # ✅ FIXED: unique per row via zip
                    "supplier": pr_doc.supplier,
                ...
                # ✅ Insert asset
                asset_doc.insert(ignore_permissions=True)
                # frappe.db.set_value("Asset", asset_doc.name, "purchase_receipt_item", pr_item_row.name, update_modified=False)
                print("Asset inserted with name:1813=============== ", asset_doc.name,"pr_item_row.name================= ", pr_item_row.name)
                ...
                print(f"Asset {asset_doc.name} created and submitted successfully!========asset_doc.name================= ", asset_doc.purchase_receipt_item)
                # ✅ Update Asset Master with MR/PO/PR references
                frappe.db.set_value("Asset", asset_doc.name, "purchase_receipt_item", pr_item_row.name, update_modified=False)
                frappe.db.commit()
                # frappe.db.set_value("Asset", asset_doc.name, "purchase_receipt_item", pr_item_row.name, update_modified=False)
                print(f"Asset {asset_doc.name} created and submitted successfully!========asset_doc.name=========1825-------------======== ", asset_doc.purchase_receipt_item)
...
@frappe.whitelist()
def get_supplier_document_details(doc):
...
def link_qi_and_ar_no(doc, method=None):
    """Link QI and create Serial & Batch Bundle with AR numbers."""
    errors = []
    ...
    for item in doc.items:
        ...
        if not qi_list:
            errors.append(
                f"Row #{item.idx} | Item <b>{item.item_code} - {item.item_name}</b>: "
                f"Quality Inspection not submitted."
            )
            continue

        qi_name = qi_list[0].name

        # 2. Fetch AR Numbers for this QI + item
        ar_records = frappe.db.get_all(
            "Analytic Number",
            filters={
                "quality_insepction": qi_name,
                "item_code": item.item_code,
                "purchase_receipt": doc.name,
                "enabled": 1
            },
            fields=["analytical_no"],
            order_by="qty_sequence asc"
        )

        if not ar_records:
            errors.append(
                f"Row #{item.idx} | Item <b>{item.item_code}</b>: "
                f"No AR Numbers found for QI {qi_name}."
            )
            continue

        # 3. Create Serial Numbers in ERPNext if they don't exist
        for ar in ar_records:
            if not frappe.db.exists("Serial No", ar.analytical_no):
                sn_doc = frappe.new_doc("Serial No")
                sn_doc.serial_no = ar.analytical_no
                sn_doc.item_code = item.item_code
                sn_doc.insert(ignore_permissions=True)

        # 4. Create Serial and Batch Bundle
        bundle = frappe.new_doc("Serial and Batch Bundle")
        bundle.item_code = item.item_code
        bundle.warehouse = item.warehouse
        ...
        serial_nos_text = "\n".join([r.analytical_no for r in ar_records])
        ...
        item.quality_inspection = qi_name
        item.serial_no = serial_nos_text         
    if errors:
        frappe.throw(
            title="Cannot Submit Purchase Receipt",
            msg="<br><br>".join(errors)
        )

def validate_qi(doc, method=None):
    """Pre-link QI to item rows so ERPNext core validate_inspection passes."""
    for item in doc.items:
        ...
        if qi_list:
            item.quality_inspection = qi_list[0].name
            frappe.db.set_value(
                "Purchase Receipt Item",
                item.name,
                "quality_inspection",
