...
def make_stock_entry(source_name, target_doc=None):
	...
	def update_item(obj, target, source_parent):
		...
		target.conversion_factor = obj.conversion_factor
		if obj.custom_operation:
			target.custom_operation = obj.custom_operation
	...
	def set_missing_values(source, target):
		...
		print(" source.work_order----------------677", source.work_order)
        
		if source.job_card or source.work_order:
			print("-----679--------")
			target.purpose = "Material Transfer for Manufacture"
			target.plant = source.custom_plant
		...
		if source.job_card or source.work_order:
			...
			if not job_card_details:
				work_order_details = frappe.get_all(
					"Work Order", filters={'name': source.work_order}, fields=['bom_no', 'qty']
				)
			...
			if job_card_details and job_card_details[0]:
				...
				target.from_bom = 1
			if work_order_details and work_order_details[0]:
				target.bom_no = work_order_details[0].bom_no
				if work_order_details[0].get('material_transferred_for_manufacturing'):
					target.fg_completed_qty = 0
				else:
					target.fg_completed_qty = work_order_details[0].qty
				target.from_bom = 1
