...
frappe.ui.form.on("Job Card", {
  ...
  add_qi_button_if_required(frm)
    ...
    if (
      frm.is_new() ||
      frm.doc.__islocal ||
      !frm._software_fields_initialized
    ) {
      setTimeout(() => {
...
before_save(frm) {
  ...
  custom_feasibility_testing_template: function (frm) {
    ...
    if (frm.doc.custom_feasibility_testing_template) {
      ...
      load_feasibility_testing_from_template(
        frm,
        frm.doc.custom_feasibility_testing_template
      )
        .then(() => {
  ...
  custom_line_clearance_template: function (frm) {
    if (frm._loading_line_clearance_template || frm._refresh_in_progress)
      return;

    if (frm.doc.custom_line_clearance_template) {
      frm._loading_line_clearance_template = true;
      load_line_clearnce_from_template(
        frm,
        frm.doc.custom_line_clearance_template
      )
        .then(() => {
...
function manage_tab_visibility(frm) {
  ...
  frm.set_df_property(
    "custom_feasibility_testing",
    "hidden",
    hide_feasibility ? 1 : 0
  );
  frm.set_df_property(
    "custom_feasibility_test",
    "hidden",
    hide_feasibility || !has_feasibility_data ? 1 : 0
  );

  // Operations List - Hide if: software operation OR draft state OR no operation details
  frm.set_df_property(
    "custom_opeartions_list",
    "hidden",
  ...
  const hide_line_clearance =
    !has_line_clearance_template || !has_line_clearance_data;
  frm.set_df_property(
    "custom_line_clearance_checklist_details",
    "hidden",
    hide_line_clearance ? 1 : 0
  );
  frm.set_df_property(
    "custom_line_clearance",
    "hidden",
    hide_line_clearance ? 1 : 0
  );
...
function load_feasibility_testing_template(frm) {
  ...
  return frappe.db
    .get_doc("Operation", frm.doc.operation)
    .then((operation_doc) => {
      let template_name =
        operation_doc.custom_feasibility_testing_template || "";
      ...
      return load_feasibility_testing_from_template(
        frm,
        template_name,
        operation_doc
      ).then(() => {
...
function load_feasibility_testing_from_template(
  frm,
  template_name,
  operation_doc = null
) {
  return frappe.db
    .get_doc("Feasibility Testing Template", template_name)
    .then((template) => {
      frm.clear_table("custom_feasibility_testing");

      (template.feasibility_testing_template_details || []).forEach(
        (template_row) => {
          ...
          if (
            operation_doc &&
            operation_doc.custom_feasibility_testing_details
          ) {
            let existing_row =
              operation_doc.custom_feasibility_testing_details.find(
                (op_row) =>
                  op_row.feasibility_testing ===
                  template_row.feasibility_testing
              );
...
function load_line_clearance_template(frm) {
  ...
  return frappe.db
    ...
    .then((operation_doc) => {
      ...
      return load_line_clearnce_from_template(
        frm,
        template_name,
        operation_doc
      ).then(() => {
...
function load_line_clearnce_from_template(
  frm,
  template_name,
  operation_doc = null
) {
  return frappe.db
    .get_doc("Line Clearance Template", template_name)
    .then((template) => {
      frm.clear_table("custom_line_clearance_checklist_details");

      (template.line_clearance_template_details || []).forEach(
        (template_row) => {
          let child = frm.add_child("custom_line_clearance_checklist_details");
          child.line_clearance_checklist =
            template_row.line_clearance_checklist;
        }
      );
...
frappe.ui.form.on("Job Card Opeartion Deatils", {
  batch_number: function (frm, cdt, cdn) {
    ...
    if (row.batch_number) {
      frappe.db
        .get_value("Batch", row.batch_number, "custom_batch_number")
        .then((r) => {
