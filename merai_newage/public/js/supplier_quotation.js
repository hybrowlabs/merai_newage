	...
	refresh: function (frm) {
        ...
        if ((frm.doc.custom_is_revision && frm.doc.custom_previous_quotation) || frm.doc.amended_from ){
            ...
            if (!frm.custom_compare_added) {
                ...
                frm.custom_compare_added = true;

                frm.add_custom_button("Compare Quotation", function () {
                    show_comparison(frm);
                });
            }
        }
...
function set_requisitioner_from_reference(frm) {
	...
	frappe.db
		.get_value("Supplier Quotation Item", { parent: sq }, ["material_request"])
		.then((r) => {
...
function toggle_base_rate_reqd(frm) {
	...
	frm.fields_dict.items.grid.update_docfield_property("base_rate", "reqd", is_required ? 1 : 0);
